# Totally stole this from: https://github.com/akhayyat/pelican-page-hierarchy
...
def override_metadata(content_object):
    ...
    current_url = getattr(content_object, 'override_url', None) or content_object.url
    save_as = getattr(content_object, 'override_save_as', None) or content_object.save_as

    # Okay, so I really didn't like the static pages having /pages/ in the URL, so this is a bit of a hack
    if type(content_object) in (contents.Page, contents.Static):
        if save_as.startswith('pages/'):
            content_object.override_save_as = save_as[6:]
        if current_url.startswith('pages/'):
            content_object.override_url = current_url[6:]
